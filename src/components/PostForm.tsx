...
const sectionTitleClass = "text-xs font-semibold tracking-wide text-muted-foreground";
...
export default function PostForm({
  ...
  const roastIndex = useMemo(() => {
    const index = mstData.roast_levels_jp.findIndex((level) => level === form.roast_level);
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      roast_date: initialData.roast_date ? initialData.roast_date.slice(0, 10) : "",
        ...
        <DialogContent className="h-[100svh] w-full max-w-none flex flex-col gap-0 rounded-none border-0 p-0 sm:h-auto sm:max-h-[92svh] sm:max-w-[560px] sm:rounded-2xl sm:border overflow-hidden">
          <DialogHeader className="px-5 pt-6 pb-4 border-b bg-background/95">
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1">
              <Info className="h-3.5 w-3.5" /> * は必須項目です（詳細はあとから編集できます）
            </p>
          </DialogHeader>
            ...
            <form id="mamelog-form" onSubmit={handleSubmit} className="space-y-7 pt-5">
              <section className="space-y-4 rounded-2xl border bg-card p-4 sm:p-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2 sm:col-span-1">
                    <Label className="mb-2 text-gray-700 flex items-center gap-2" htmlFor="volume">
                      <Weight className="w-4 h-4 text-primary/80" />
                      内容量(g)
                    </Label>
                    <Input
                      id="volume"
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {volumePresets.map((preset) => (
                        <button
                          key={preset}
                          type="button"
                          onClick={() => setForm((prev) => ({ ...prev, volume: preset }))}
                ...
                <div>
                  <Label className="mb-2 text-gray-700 flex items-center gap-2" htmlFor="shop_name">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2 sm:col-span-1">
                    <Label className="mb-2 text-gray-700 flex items-center gap-2" htmlFor="price">
                      <BadgeJapaneseYen className="w-4 h-4 text-primary/80" />
                      価格(円)
                    </Label>
                    <Input
                      id="price"
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {pricePresets.map((preset) => (
                        <button
                          key={preset}
                          type="button"
                          onClick={() => setForm((prev) => ({ ...prev, price: preset }))}
                          className={`px-2.5 py-1 text-xs rounded-full transition ${
                            form.price === preset
                              ? "bg-primary text-white"
                              : "bg-muted text-muted-foreground hover:bg-muted/70"
                ...
                <div className="space-y-2">
                  ...
                  <div className="rounded-xl bg-muted/45 px-3 py-4">
                    <input
                      ...
                      type="range"
                      min={0}
                      max={mstData.roast_levels_jp.length - 1}
                      value={roastIndex}
                      onChange={(e) => {
                        const level = mstData.roast_levels_jp[Number(e.target.value)];
                        setForm((prev) => ({ ...prev, roast_level: level }));
                        setErrors((prev) => ({ ...prev, roast_level: false }));
                      }}
                      className="w-full accent-[#7a3f24]"
                    />
                    <div className="flex items-center justify-between mt-2 text-[11px] text-muted-foreground">
                      <span>浅煎り</span>
                      <span className="text-primary font-semibold">{form.roast_level}</span>
                      <span>深煎り</span>
                    </div>
                  </div>
                </div>
              ...
              <section className="space-y-4 rounded-2xl border bg-card p-4 sm:p-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2 sm:col-span-1">
                    <Label className="mb-2 text-gray-700 flex items-center gap-2" htmlFor="generation">
                ...
                <div>
                  <Label className="mb-2 text-gray-700 flex items-center gap-2" htmlFor="farm">
                  ...
                  <Input
                    ...
                    maxLength={30}
                  />
                  <p className="text-right text-xs text-gray-400 mt-1">{form.farm.length}/30</p>
                </div>
              </section>
              ...
              <Collapsible open={isNicheOpen} onOpenChange={setIsNicheOpen}>
                ...
                <CollapsibleContent className="mt-3 space-y-4 rounded-2xl border bg-card p-4 sm:p-5">
                  ...
                  <div>
                    <Label className="mb-2 text-gray-700 flex items-center gap-2" htmlFor="district_name">
                  ...
                  <div>
                    <Label className="mb-2 text-gray-700 flex items-center gap-2" htmlFor="flavor">
                  ...
                  <div>
                    <Label className="mb-2 text-gray-700 flex items-center gap-2" htmlFor="comment">
                  ...
                  <div>
                    <Label className="mb-2 text-gray-700 flex items-center gap-2" htmlFor="exp_date">
